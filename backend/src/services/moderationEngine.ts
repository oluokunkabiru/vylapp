...
const PATTERN_BANK: PatternRule[] = [
...
function layer1Check(content: string) {
  const normalized = normalizeUnicode(content);
  const flags: { category: string; lang: string; weight: number }[] = [];
...
async function layer2Check(content: string, lang: string) {
...
const ModerationEngine = {
  ...
  async analyzeContent(content: string | null | undefined, context: ModerationContext = {}) {
  ...
  analyzeContentSync(content: string | null | undefined, context: ModerationContext = {}) {
    if (!content?.trim()) return { category: "SAFE", confidence: 1, action: "allow", flags: [], method: "empty" };
    const detectedLang = context.language || detectLanguage(content);
    const { topCategory, maxWeight, flags } = layer1Check(content);
    let confidence = maxWeight;
  ...
  trustScore(account: { age_days?: number; email_verified?: boolean; phone_verified?: boolean; violations?: number; verified_creator?: boolean }) {
    let score = 50;
  ...
  reviewAppeal(appeal: { evidence?: unknown }, originalDecision: { confidence: number; category: string }) {
