const TIERS = {
  new_user: { label: "New User", min_points: 0, badge: "🌱", perks: [] as string[] },
...
const RavenEngine = {
  ...
  getTier(points: number) {
    let key: keyof typeof TIERS, tier: typeof TIERS[keyof typeof TIERS];
  ...
  pointsFor(action: string, multiplier = 1): number {
