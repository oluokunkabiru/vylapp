...
const FROM_ISO6393 = Object.fromEntries(Object.entries(TO_ISO6393).map(([k, v]) => [v, k]));
...
function loadFranc() {
  if (!francPromise) francPromise = import("franc-min").then(m => m.franc);
  return francPromise;
}

const LanguageDetector = {
  // Returns a 2-letter code from TranslationEngine.LANGUAGES, or `fallbackLang` if unsure.
  async detect(text, fallbackLang = "en") {
