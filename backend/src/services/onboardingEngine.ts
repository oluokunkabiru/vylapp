...
const FIRST_VIBE_PROMPTS: Record<string, string> = {
...
const OnboardingEngine = {
  ...
  matchCreators(interests: string[], creatorPool: any[]) {
  ...
  generateFirstVibePrompt(interests: string[] = []): string {
    const key = interests[0] || "global";
    return FIRST_VIBE_PROMPTS[key] || FIRST_VIBE_PROMPTS.global;
  },

  completionScore(events: { step: string }[]) {
    const weights: Record<string, number> = { welcome: 10, interests: 20, handle: 15, avatar: 10, location: 15, follow_suggestions: 15, complete: 15 };
