    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={e => e.stopPropagation()}>
        ...
        {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 50 }}><Spinner size={28} /></div> : u && <>
          ...
          <section style={sectionStyle}><SectionTitle title={`Recent vibes (${detail.content.vibes.length})`} />{detail.content.vibes.map(v => <div key={v.id} style={rowStyle}><a href={`/admin/content/vibes/${v.id}`} style={{ color: "var(--text)", textDecoration: "none", flex: 1 }}>{v.content.slice(0, 180) || "Media-only vibe"}</a><span style={{ color: "var(--text3)", fontSize: 11 }}>{v.likesCount} likes · {v.repliesCount} replies</span></div>)}{!detail.content.vibes.length && <EmptyLine text="No vibes published" />}</section>
