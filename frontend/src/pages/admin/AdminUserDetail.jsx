...
export default function AdminUserDetail() {
  ...
  return <div style={{ padding: "28px 32px 60px", maxWidth: 1120 }}>
    ...
    <section id="vibes" style={section}><div style={title}>Vibes — {totals.vibes} total</div>{content.vibes.map(vibe => <div key={vibe.id} style={row}><a href={`/admin/content/vibes/${vibe.id}`} style={{ color: "var(--text)", textDecoration: "none", flex: 1 }}>{vibe.content || "Media-only vibe"}</a><span style={muted}>{vibe.likesCount} likes · {vibe.repostsCount} reshares · {vibe.repliesCount} replies · {vibe.viewsCount} views</span></div>)}{!content.vibes.length && <Empty text="No vibes published" />}</section>
