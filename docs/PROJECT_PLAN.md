...
- [ ] D-10 Primitives: tabs, menu, chip, badge — not built. `CategoryPill` covers chip-like usage for one specific case; no generic Chip/Badge/Tabs/Menu primitive exists. Deliberately not stubbed out without a real screen to wire them into (would be unused speculative code).
...
- [ ] D-15 RTL mirroring system — layout, icons, gestures. Not started (see the i18n-infrastructure gap noted under D-02 above — this depends on it for anything beyond per-element `dir`).
